...
RAG_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        "Trả lời ngắn gọn trong 1–3 câu. Nếu context không có thông tin, hãy nói "
...
# ── 5. Main ────────────────────────────────────────────────────────────────
def parse_args():
    parser = argparse.ArgumentParser(description="Run the Part 1 LangSmith RAG pipeline.")
    parser.add_argument(
        "--limit",
        type=int,
        default=len(SAMPLE_QUESTIONS),
        help="Number of questions to run (default: all 50).",
    )
    args = parser.parse_args()
    if not 1 <= args.limit <= len(SAMPLE_QUESTIONS):
        parser.error(f"--limit must be between 1 and {len(SAMPLE_QUESTIONS)}")
    return args
...
def main(limit: int):
    ...
    print(f"Đang chạy {len(questions)} câu hỏi qua RAG pipeline...")
    for i, question in enumerate(questions, 1):
        # Each invocation is a new top-level @traceable run. The runtime name and
        # metadata make the 50 dashboard entries easy to identify and filter.
        answer = ask(
            chain,
            question,
            langsmith_extra={
                "name": f"rag_query_{i:03d}",
                "metadata": {"question_id": i, "question": question},
            },
        )
        print(f"[{i:02d}/{len(questions)}] Q: {question[:60]}")
        print(f"       A: {str(answer)[:100]}\n")

    print(f"\n✅ {len(questions)} traces đã gửi lên LangSmith project '{config.LANGSMITH_PROJECT}'")
