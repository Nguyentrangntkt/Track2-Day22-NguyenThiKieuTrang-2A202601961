...
PROMPT_V1_NAME = "day22-lab-rag-concise-v1"
PROMPT_V2_NAME = "day22-lab-rag-structured-v2"
...
def push_prompts_to_hub(client: Client):
    ...
    try:
        url = client.push_prompt(
            PROMPT_V1_NAME,
            object=PROMPT_V1,
            description="V1: trả lời RAG ngắn gọn, thân thiện (2–4 câu)",
        )
    ...
    try:
        url = client.push_prompt(
            PROMPT_V2_NAME,
            object=PROMPT_V2,
            description="V2: trả lời RAG chuyên gia, có cấu trúc (3–5 câu)",
        )
...
# ── 8. Main ────────────────────────────────────────────────────────────────
def parse_args():
    parser = argparse.ArgumentParser(description="Run deterministic Part 2 A/B routing.")
    parser.add_argument(
        "--limit",
        type=int,
        default=len(SAMPLE_QUESTIONS),
        help="Number of questions to route (default: all 50).",
    )
...
def main(limit: int):
    ...
    for i, question in enumerate(questions, 1):
        ...
        if version_tag == "v1":
            ...
            v2_count += 1
        print(f"[{i:02d}/{len(questions)}] [{request_id}] [v{version_tag[-1]}]")
        print(f"  Q: {result['question']}")
        print(f"  A: {result['answer']}\n")

    print(f"\n📊 Routing: V1={v1_count} câu | V2={v2_count} câu | Tổng={len(questions)}")
