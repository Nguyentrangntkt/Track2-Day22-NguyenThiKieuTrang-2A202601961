...
class PIIDetector(Validator):
    ...
    # guardrails-ai==0.6.7 only applies a PassResult.value_override when this is set.
...
class JSONFormatter(Validator):
    ...
    """
    # Allow the formatted JSON returned by PassResult to replace the original text.
    override_value_on_pass = True
    ...
    def validate(self, value: str, metadata: dict):
        ...
        Nếu JSON không thể sửa được, trả về fallback JSON hợp lệ.
        """
        ...
        try:
            ...
            # Theo Guide: không thể sửa thì thay bằng JSON lỗi chuẩn, luôn parse được.
            fallback = json.dumps(
                {
            ...
            return PassResult(value_override=fallback)


def _guard_output(guard: Guard, result) -> str:
    """Return the final local validator value recorded by Guardrails.

    In guardrails-ai==0.6.7, a custom PassResult value override is recorded in
    the validator log but is not propagated to ValidationOutcome.validated_output
    for Guard().use(...).validate(...).  Reading the log keeps this demo on the
    scaffold's Guard API while displaying the value the validator actually made.
    """
    validator_logs = guard.history.last.iterations.last.outputs.validator_logs
    if validator_logs:
        return str(validator_logs[-1].value_after_validation)
...
def demo_pii_guard():
    print("\n" + "=" * 55)
    print("  PII DEMO — Detection & Masking")
    print("=" * 55)
    ...
    for label, text in test_cases:
        ...
        output = _guard_output(guard, result)
        input_has_pii = any(re.search(pattern, text) for pattern in PIIDetector.PII_PATTERNS.values())
        output_has_pii = any(re.search(pattern, output) for pattern in PIIDetector.PII_PATTERNS.values())
        masked = input_has_pii and not output_has_pii
        status = "✅ MASKED" if masked else ("✅ CLEAN" if not input_has_pii else "❌ PII EXPOSED")
...
def demo_json_guard():
    ...
    for label, text in test_cases:
        ...
        if result.validation_passed:
            json.loads(output)
            print("  Parse check: ✅ json.loads passed")
        else:
            print("  Parse check: not applicable (unrepairable input)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the Guardrails AI Part 4 demos.")
    parser.add_argument(
        "--demo",
        choices=("pii", "json", "all"),
        default="all",
        help="Run only the PII or JSON evidence demo (default: all).",
    )
